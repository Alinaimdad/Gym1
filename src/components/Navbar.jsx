...
const Navbar = () => {
  ...
  // List of pages where navbar text should be white
  const whiteTextPages = ['/signin', '/signup', '/forgot-password', '/payments'];

  // Check if current page is in that list
  const isWhiteTextPage = whiteTextPages.includes(location.pathname);

  // Use white or black text based on page
  const textColorClass = isWhiteTextPage ? 'text-white' : 'text-black';
